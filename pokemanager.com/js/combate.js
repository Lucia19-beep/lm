...
if (poke2.Type && poke2.Type.length > 0) {
    ...
    fetch("combate.php")
        ...
        .then(txt => {
            ...
            // Convertir HP a numero y asegurarse que Type es array
            for (let i = 0; i < data.jugador.length; i++) {
                data.jugador[i].HP = parseFloat(data.jugador[i].HP);
                if (!Array.isArray(data.jugador[i].Type)) {
                    data.jugador[i].Type = [data.jugador[i].Type];
                }
            }
            for (let i = 0; i < data.rival.length; i++) {
                data.rival[i].HP = parseFloat(data.rival[i].HP);
                if (!Array.isArray(data.rival[i].Type)) {
                    data.rival[i].Type = [data.rival[i].Type];
                }
            }
